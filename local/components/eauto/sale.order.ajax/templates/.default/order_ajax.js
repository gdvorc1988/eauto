	...
	BX.Sale.OrderAjaxComponent = {
		...
		getBlockFooter: function(node)
		{
			...
			if (!isLastNode)
			{
				buttons.push(
					BX.create('button', {
						props: {href: 'javascript:void(0)', className: 'bx_next_btn'},
		...
		editFadeRegionContent: function(node)
		{
			...
			var selectedPersonType = this.getSelectedPersonType(),
				...
				fadeParamName, i, k, locationString, Installation_service=null, validRegionErrors;
			...
			for (i in this.result.ORDER_PROP.properties)
			{
				if (this.result.ORDER_PROP.properties.hasOwnProperty(i))
				{
					...
					{
						//console.log( this.result.ORDER_PROP.properties[i] );
						...
						if ( this.result.ORDER_PROP.properties[i].CODE == 'Installation_service' ){
							Installation_service = this.result.ORDER_PROP.properties[i];
						}
					}*/
				}
			}
			
			/*if (Installation_service && Installation_service != null){
				console.log(Installation_service);
				addedHtml += '<strong>' + BX.util.htmlspecialchars(Installation_service.NAME) + ':</strong> '
					+ '-' + '<br>';
			}*/
