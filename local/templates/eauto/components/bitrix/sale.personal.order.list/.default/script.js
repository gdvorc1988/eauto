...
(function() {
	BX.Sale.PersonalOrderComponent.PersonalOrderList = {
		...
		{
			var rowWrapper = document.getElementsByClassName('js-list-inner-row');
			...
			{
				var shipmentTrackingId = wrapper.getElementsByClassName('js-shipment-id');
				if (shipmentTrackingId[0])
				{
					Array.prototype.forEach.call(shipmentTrackingId, function(blockId)
					{
						var clipboard = blockId.parentNode.getElementsByClassName('js-shipment-id-icon')[0];
						if (clipboard)
						{
				...
				{
					var block = wrapper.getElementsByClassName('js-inner-row-body')[0];
					var template = wrapper.getElementsByClassName('js-inner-row-template')[0];
					var cancelPaymentLink = template.getElementsByClassName('js-list-cancel-payment')[0];
				...
				BX.bindDelegate(wrapper, 'click', { 'class': 'js-change-payment' }, BX.proxy(function(event)
				{
					event.preventDefault();
